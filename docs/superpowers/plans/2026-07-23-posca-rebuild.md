...
## Round 4 (user decisions 2026-07-24)

### Task N3: density, theme icon, dirty dot, control fixes
User decisions: (1) density beats zero-shift — the reserved label row collapses when both labels are off (pure dense color grid); enabling labels may reflow vertically, column count stays constant so the grid shape stays recognizable. (2) Theme control becomes a compact icon button (cycles Auto/Light/Dark, state visible via icon + aria-label/title; persistence unchanged). (3) The bar-level "not saved" indicator is replaced by a small dot on the copy button (dirty = dot visible; #dirty-indicator element may go, keep an accessible name change on the button). Fix outright: the two-line wrap bug of the old indicator (moot after removal), and the off-state of Number/Name toggles that reads as disabled — off must look interactive (consistent with the active=ink convention). Also: improve dark-mode missing-state pigment recognizability (empty-inventory first impression), shrink the sticky bar (target: meaningfully under 165px via the icon + dot changes), and address name truncation in name-mode (prefer 2-line wrap with ellipsis fallback; no mid-word hard cuts for the ~26 affected names). Mandatory: impeccable + ui-ux-pro-max + frontend-design loaded before code; sequential-thinking for the control-model changes; playwright-cli verification (regression set, label matrix incl. collapse behavior, theme cycle from icon, dirty dot none→visible→none, bar height measured, dark screenshots). Only index.html.

---

## Round 5 (user requests 2026-07-24)

### Task N4: ownership filter
User request: a toggle to filter the grid by ownership — three states: All (default), Owned only, Missing only. One control in the header, same component vocabulary as Number/Name toggles. Filtering hides non-matching swatches; category subgroups and size sections that end up empty are hidden entirely EXCEPT the size section header + counter (counter always shows "N of M" for the full size so context stays). Filter state NOT persisted (in-store quick toggle, default All on load). Grid density/columns unchanged. DOM contract intact; only index.html; skill gate impeccable + ui-ux-pro-max + frontend-design; playwright-cli verification (filter matrix x label matrix, counters unchanged by filter, empty-group hiding, regression set).

### Task N5: resolve remaining 7 unknown numbers
Attempt further sources (uni-ball UK/EU shops, cultpens product pages, additional mpuni.co.jp range pages e.g. PC-5BR/PCF-350/PC-1MR if they exist). NEW sanctioned identity rule (documented): when two independent saved sources list a color in the SAME size range with the IDENTICAL hex we scraped from posca.com but under a name variant (e.g. posca.com "Coral" vs sources "Coral Pink", "Yellow Fluo" vs "Fluorescent Yellow"), the number may be accepted with the name variant + both provenances documented in the report and a code comment; display name stays the posca.com verbatim (normalized) name. Anything still unresolved stays null and gets listed for Philip to check on physical caps in-store (definitive primary source).

### Task N6: physical cap evidence (owner-provided, 2026-07-24)
Philip read the actual caps: PC-3M has BOTH nr 21 "Brown" and nr 22 "Dark Brown" (so posca.com's second 3M "Brown" #572d2d = Dark Brown 22; Jenny's hex match for 22 = #572d2d corroborates); PC-5M nr 66 is named "Coral Pink" on the cap (posca.com's only 5M coral, hex #ff8da1 → Coral = Coral Pink 66; propagates to 5BR via the name-keyed map like every other color); PC-5M F2 cap named "F.Yellow" (posca.com's only 5M fluo yellow "Yellow Fluo" → F2; display name becomes "Fluorescent Yellow" for consistency with sibling Fluorescent Orange/Pink/Red and both chart sources). Physical cap = primary source, recorded as "physical cap (owner), 2026-07-24" in report + code comment.
Data: add a hex-qualified/name correction mechanism in build-lineup.py (scrape name+hex → corrected display name), number-map entries coral→66, yellow fluo→F2, dark-brown 22 for the #572d2d entry. Keys change (3M:Brown-2→3M:Dark Brown, 5M:Coral→5M:Coral Pink, 5BR:Coral→5BR:Coral Pink, 5M:Yellow Fluo→5M:Fluorescent Yellow) — inventory.js is empty so no committed migration, but index.html's cleanStaleOverlay must additionally prune overlay keys that do not exist in LINEUP (protects Philip's local ticks from orphaned renamed keys). Remaining nulls after this: 1MR Glacier Blue, 5M Grape Green, 8K Pale Fluorescent Orange (cap check pending).
